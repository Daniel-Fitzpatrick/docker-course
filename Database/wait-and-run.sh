...
for i in {1..50};
do
    /opt/mssql-tools18/bin/sqlcmd -C -S database -U sa -P Password1234 -Q "SELECT 1" > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
...
# Run the SQL script
/opt/mssql-tools18/bin/sqlcmd -C -S database -U sa -P Password1234 -d master -i /CreateDatabaseAndSeed.sql
